...
import { Check, Plus, X, Settings, Moon, Sun, Bell, BellOff, ChevronRight, Calendar, TrendingUp, Hourglass, Info, Download, Upload, QrCode, Camera } from 'lucide-react';
...
const HabitTracker = () => {
  ...
  const getLifeProgress = () => {
    ...
    return Math.min((currentAge / settings.lifeExpectancy) * 100, 100);
  };
          ...
          <div className="mb-8 flex items-center gap-4">
            ...
            <div className="flex-1 relative">
              ...
              {getRetirementProgress() && (
                <>
                  <div 
                    className={`absolute bottom-full mb-1 w-0.5 h-2 ${darkMode ? 'bg-white' : 'bg-black'}`}
                    style={{ left: `${getRetirementProgress()}%` }}
                  />
                  <div 
                    className={`absolute top-full mt-1 w-0.5 h-2 ${darkMode ? 'bg-white' : 'bg-black'}`}
                    style={{ left: `${getRetirementProgress()}%` }}
                  />
                </>
              )}
              <div className={`h-2 rounded-full overflow-hidden ${darkMode ? 'bg-zinc-900' : 'bg-zinc-200'}`}>
                <div 
                  className={`h-full transition-all duration-1000 ${darkMode ? 'bg-white' : 'bg-black'}`}
                  style={{ width: `${getLifeProgress()}%` }}
                />
              </div>
            </div>
          </div>
                  ...
                  >
                    ...
                    <div className={`w-20 flex items-center justify-center shrink-0 ${
                      ...
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <div className={`text-xs leading-none mt-0.5 ${darkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>
                    ...
                    <div className="flex-1 p-5">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          ...
                          <span className={`text-xs mt-1 inline-block ${darkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>
                            {habit.frequency === 'daily' ? 'Daily' : habit.frequency === 'every2days' ? 'Every 2 days' : habit.frequency === 'weekly' ? 'Weekly' : 'Monthly'}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {habit.completions.slice(-50).map((completion, idx) => (
                          <div
                            key={idx}
